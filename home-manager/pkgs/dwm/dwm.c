...
enum { SchemeNorm, SchemeSel, SchemeTitle, SchemeUnderline }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeUnderline : SchemeNorm]);/*delete this line if it doesn't work*/
...
manage(Window w, XWindowAttributes *wa)
{
	...
	/* Child dialogs/popups should not inherit the normal client border. */
	c->bw = trans != None ? 0 : borderpx;
